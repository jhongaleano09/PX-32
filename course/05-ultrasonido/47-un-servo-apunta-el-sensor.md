...
Hoy vas a **mover el MG90 a 45°, 90° y 135° sin forzar topes**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 17: PWM: regular energía en el tiempo](../02-movimiento/17-pwm-regular-energia-en-el-tiempo.md), [Lección 46: Medir objetos conocidos](46-medir-objetos-conocidos.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **servo, ángulo, posición y señal de control**. En lenguaje cotidiano, buscamos una forma fiable de mover el MG90 a 45°, 90° y 135° sin forzar topes. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Servo:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres mover el MG90 a 45°, 90° y 135° sin forzar topes y qué mecanismo produciría ese resultado?
...
1. **Preparar.** Coloca PX-32 estable, identifica HW-010 y confirma con el adulto que la energía está en el estado seguro. Continúa solo si no hay cables sueltos, daño, calor u olor.
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con servo, ángulo, posición y señal de control. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a cargar posiciones con pausa y verificar orientación física. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: tres posiciones distintas y ausencia de zumbido continuo o golpes. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [47-un-servo-apunta-el-sensor.ino](../../code/educational/47-un-servo-apunta-el-sensor/47-un-servo-apunta-el-sensor.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **servo**. Lee el programa de arriba abajo y predice su salida.
...
El resultado normal es **tres posiciones distintas y ausencia de zumbido continuo o golpes**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **servo** y dónde aparece en PX-32.
...
Hoy aprendiste a **mover el MG90 a 45°, 90° y 135° sin forzar topes** y lo conectaste con **servo, ángulo, posición y señal de control**. Pudiste observar tres posiciones distintas y ausencia de zumbido continuo o golpes. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 48: Escaneo espacial](48-escaneo-espacial.md).
