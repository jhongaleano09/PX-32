...
Hoy vas a **convertir duración de eco en una estimación de distancia**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 44: ECHO: medir una duración](44-echo-medir-una-duracion.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **unidad, conversión, constante y distancia**. En lenguaje cotidiano, buscamos una forma fiable de convertir duración de eco en una estimación de distancia. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Unidad:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres convertir duración de eco en una estimación de distancia y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con unidad, conversión, constante y distancia. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a aplicar distancia = duración×0,0343/2 y revisar unidades. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: valores cercanos a una referencia, con margen por geometría y ambiente. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [45-de-microsegundos-a-centimetros.ino](../../code/educational/45-de-microsegundos-a-centimetros/45-de-microsegundos-a-centimetros.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **unidad**. Lee el programa de arriba abajo y predice su salida.

```cpp
// Curso PX-32 — programa mínimo de la lección 45
...
El resultado normal es **valores cercanos a una referencia, con margen por geometría y ambiente**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **unidad** y dónde aparece en PX-32.
...
Hoy aprendiste a **convertir duración de eco en una estimación de distancia** y lo conectaste con **unidad, conversión, constante y distancia**. Pudiste observar valores cercanos a una referencia, con margen por geometría y ambiente. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 46: Medir objetos conocidos](46-medir-objetos-conocidos.md).
