...
Hoy vas a **evaluar la calidad de varias mediciones ultrasónicas**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 08: PX-32 aprende a hablarnos](../01-programacion/08-px-32-aprende-a-hablarnos.md), [Lección 45: De microsegundos a centímetros](45-de-microsegundos-a-centimetros.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **precisión, exactitud, repetición y mediana**. En lenguaje cotidiano, buscamos una forma fiable de evaluar la calidad de varias mediciones ultrasónicas. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Precisión:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres evaluar la calidad de varias mediciones ultrasónicas y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con precisión, exactitud, repetición y mediana. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a medir tres distancias conocidas cinco veces cada una. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: variación pequeña, valores atípicos identificados y límites cercanos explicados. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [46-medir-objetos-conocidos.ino](../../code/educational/46-medir-objetos-conocidos/46-medir-objetos-conocidos.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **precisión**. Lee el programa de arriba abajo y predice su salida.

```cpp
// Curso PX-32 — programa mínimo de la lección 46
...
El resultado normal es **variación pequeña, valores atípicos identificados y límites cercanos explicados**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **precisión** y dónde aparece en PX-32.
...
Hoy aprendiste a **evaluar la calidad de varias mediciones ultrasónicas** y lo conectaste con **precisión, exactitud, repetición y mediana**. Pudiste observar variación pequeña, valores atípicos identificados y límites cercanos explicados. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 47: Un servo apunta el sensor](47-un-servo-apunta-el-sensor.md).
