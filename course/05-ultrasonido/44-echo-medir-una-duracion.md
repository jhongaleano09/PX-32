...
Hoy vas a **medir en D31 cuánto dura el eco sin bloquear indefinidamente**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 43: TRIG: enviar un pulso breve](43-trig-enviar-un-pulso-breve.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
- **Pulsein:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres medir en D31 cuánto dura el eco sin bloquear indefinidamente y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con pulseIn, temporización y timeout. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a usar pulseIn con tiempo límite y mostrar microsegundos. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: duraciones variables con la distancia y cero cuando vence el timeout. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [44-echo-medir-una-duracion.ino](../../code/educational/44-echo-medir-una-duracion/44-echo-medir-una-duracion.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **pulseIn**. Lee el programa de arriba abajo y predice su salida.
...
- Cuéntale con tus palabras qué significa **pulseIn** y dónde aparece en PX-32.
...
Hoy aprendiste a **medir en D31 cuánto dura el eco sin bloquear indefinidamente** y lo conectaste con **pulseIn, temporización y timeout**. Pudiste observar duraciones variables con la distancia y cero cuando vence el timeout. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 45: De microsegundos a centímetros](45-de-microsegundos-a-centimetros.md).
