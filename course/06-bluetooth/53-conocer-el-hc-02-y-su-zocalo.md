# Lección 53 — Conocer el HC-02 y su zócalo

## 1. Tu misión de hoy

Hoy vas a **identificar el HC-02 sin confundirlo con la imagen errónea de HC-05**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 02: Inventario razonado de PX-32](../00-fundamentos/02-inventario-razonado-de-px-32.md), [Lección 52: Qué hace Bluetooth](52-que-hace-bluetooth.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **módulo, firmware, zócalo y antena**. En lenguaje cotidiano, buscamos una forma fiable de identificar el HC-02 sin confundirlo con la imagen errónea de HC-05. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Módulo:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres identificar el HC-02 sin confundirlo con la imagen errónea de HC-05 y qué mecanismo produciría ese resultado?
...
1. **Preparar.** Coloca PX-32 estable, identifica HW-002, HW-011 y confirma con el adulto que la energía está en el estado seguro. Continúa solo si no hay cables sueltos, daño, calor u olor.
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con módulo, firmware, zócalo y antena. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a comparar placa física, ficha y página 45 con alimentación apagada. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: identidad registrada por serigrafía; configuración interna queda sin alterar. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
El resultado normal es **identidad registrada por serigrafía; configuración interna queda sin alterar**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **módulo** y dónde aparece en PX-32.
...
Hoy aprendiste a **identificar el HC-02 sin confundirlo con la imagen errónea de HC-05** y lo conectaste con **módulo, firmware, zócalo y antena**. Pudiste observar identidad registrada por serigrafía; configuración interna queda sin alterar. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 54: UART: hablar por turnos y a la misma velocidad](54-uart-hablar-por-turnos-y-a-la-misma-velocidad.md).
