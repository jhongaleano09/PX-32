# Lección 54 — UART: hablar por turnos y a la misma velocidad

## 1. Tu misión de hoy

Hoy vas a **distinguir Serial USB de Serial1 del módulo inalámbrico**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 08: PX-32 aprende a hablarnos](../01-programacion/08-px-32-aprende-a-hablarnos.md), [Lección 53: Conocer el HC-02 y su zócalo](53-conocer-el-hc-02-y-su-zocalo.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **UART, trama, baud rate, TX y RX**. En lenguaje cotidiano, buscamos una forma fiable de distinguir Serial USB de Serial1 del módulo inalámbrico. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Uart:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres distinguir Serial USB de Serial1 del módulo inalámbrico y qué mecanismo produciría ese resultado?
...
1. **Preparar.** Coloca PX-32 estable, identifica HW-001, HW-011 y confirma con el adulto que la energía está en el estado seguro. Continúa solo si no hay cables sueltos, daño, calor u olor.
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con UART, trama, baud rate, TX y RX. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a hacer un puente lógico de caracteres entre Serial y Serial1. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: datos legibles solo cuando ambos extremos comparten velocidad y formato. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [54-uart-hablar-por-turnos-y-a-la-misma-velocidad.ino](../../code/educational/54-uart-hablar-por-turnos-y-a-la-misma-velocidad/54-uart-hablar-por-turnos-y-a-la-misma-velocidad.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **UART**. Lee el programa de arriba abajo y predice su salida.

```cpp
// Curso PX-32 — programa mínimo de la lección 54
...
El resultado normal es **datos legibles solo cuando ambos extremos comparten velocidad y formato**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **UART** y dónde aparece en PX-32.
...
Hoy aprendiste a **distinguir Serial USB de Serial1 del módulo inalámbrico** y lo conectaste con **UART, trama, baud rate, TX y RX**. Pudiste observar datos legibles solo cuando ambos extremos comparten velocidad y formato. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 55: TX y RX se cruzan](55-tx-y-rx-se-cruzan.md).
