# Lección 57 — Control Bluetooth con parada dominante

## 1. Tu misión de hoy

Hoy vas a **interpretar F/B/L/R/S con parada ante silencio o dato inválido**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 20: Vectores para mover PX-32](../02-movimiento/20-vectores-para-mover-px-32.md), [Lección 56: Recibir una letra](56-recibir-una-letra.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **comando, despacho, watchdog y STOP**. En lenguaje cotidiano, buscamos una forma fiable de interpretar F/B/L/R/S con parada ante silencio o dato inválido. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.

Comunicar requiere un emisor, un receptor, un canal y reglas compartidas. Bluetooth transporta datos por radio; UART entrega bytes entre el módulo y la Mega. Son capas diferentes. Una letra recibida tampoco es aún una orden segura: debe validarse, traducirse y caducar si deja de llegar comunicación.

### De la intuición al concepto técnico

Los términos centrales son **comando, despacho, watchdog y STOP**. No son etiquetas decorativas: cada uno nombra una relación que podremos observar. Una analogía útil es pensar en una receta: ingredientes, pasos y resultado ayudan a organizar la acción. Pero la analogía tiene límite; PX-32 no “sabe” qué desea el cocinero y un componente real responde a voltaje, tiempo, geometría y código, no a intenciones.

En PX-32, esta idea se usa para probar primero con motores deshabilitados y luego en área libre. Antes de actuar, separa cuatro preguntas: ¿qué cambiaremos?, ¿qué mantendremos igual?, ¿qué mediremos?, ¿qué resultado nos obligaría a detenernos? Ese orden convierte una demostración llamativa en un experimento. Si modificamos dos cosas a la vez, perdemos la posibilidad de saber cuál causó el cambio.
...
- **Comando:** idea principal que podrás reconocer en la actividad.
...
**Hardware:** HW-004 a HW-006, HW-011.
...
1. ¿Qué esperas observar cuando logres interpretar F/B/L/R/S con parada ante silencio o dato inválido y qué mecanismo produciría ese resultado?
...
1. **Preparar.** Coloca PX-32 estable, identifica HW-004 a HW-006, HW-011 y confirma con el adulto que la energía está en el estado seguro. Continúa solo si no hay cables sueltos, daño, calor u olor.
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con comando, despacho, watchdog y STOP. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a probar primero con motores deshabilitados y luego en área libre. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: S, comando desconocido y timeout siempre producen detención. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [57-control-bluetooth-con-parada-dominante.ino](../../code/educational/57-control-bluetooth-con-parada-dominante/57-control-bluetooth-con-parada-dominante.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **comando**. Lee el programa de arriba abajo y predice su salida.
...
El resultado normal es **S, comando desconocido y timeout siempre producen detención**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- [Conceptos básicos de Bluetooth](https://www.bluetooth.com/learn-about-bluetooth/tech-overview/) — Inglés; explicación oficial; 8 min. Aprenderás conceptos básicos de bluetooth. Esencial.
- [UART y protocolos de PX-32](../../docs/reference/protocolos.md) — Español; referencia interna; 8 min. Aprenderás uart y protocolos de px-32. Opcional.
...
- Cuéntale con tus palabras qué significa **comando** y dónde aparece en PX-32.
...
Hoy aprendiste a **interpretar F/B/L/R/S con parada ante silencio o dato inválido** y lo conectaste con **comando, despacho, watchdog y STOP**. Pudiste observar S, comando desconocido y timeout siempre producen detención. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 58: Wi-Fi y Bluetooth no son lo mismo](../07-wifi-iot/58-wi-fi-y-bluetooth-no-son-lo-mismo.md).
