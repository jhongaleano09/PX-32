# Lección 56 — Recibir una letra

## 1. Tu misión de hoy

Hoy vas a **mostrar por USB cada carácter recibido desde Bluetooth**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 55: TX y RX se cruzan](55-tx-y-rx-se-cruzan.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **byte, carácter, available y read**. En lenguaje cotidiano, buscamos una forma fiable de mostrar por USB cada carácter recibido desde Bluetooth. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Byte:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres mostrar por USB cada carácter recibido desde Bluetooth y qué mecanismo produciría ese resultado?
...
1. **Preparar.** Coloca PX-32 estable, identifica HW-011 y confirma con el adulto que la energía está en el estado seguro. Continúa solo si no hay cables sueltos, daño, calor u olor.
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con byte, carácter, available y read. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a enviar letras inocuas y observar su código/representación. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: una letra por mensaje y ninguna acción motriz todavía. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [56-recibir-una-letra.ino](../../code/educational/56-recibir-una-letra/56-recibir-una-letra.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **byte**. Lee el programa de arriba abajo y predice su salida.

```cpp
// Curso PX-32 — programa mínimo de la lección 56
...
El resultado normal es **una letra por mensaje y ninguna acción motriz todavía**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **byte** y dónde aparece en PX-32.
...
Hoy aprendiste a **mostrar por USB cada carácter recibido desde Bluetooth** y lo conectaste con **byte, carácter, available y read**. Pudiste observar una letra por mensaje y ninguna acción motriz todavía. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 57: Control Bluetooth con parada dominante](57-control-bluetooth-con-parada-dominante.md).
