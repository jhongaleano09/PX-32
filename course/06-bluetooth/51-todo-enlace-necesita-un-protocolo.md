# Lección 51 — Todo enlace necesita un protocolo

## 1. Tu misión de hoy

Hoy vas a **dibujar una comunicación completa e identificar posibles errores**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 50: Radio: otra región del espectro](50-radio-otra-region-del-espectro.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **emisor, receptor, mensaje, canal y protocolo**. En lenguaje cotidiano, buscamos una forma fiable de dibujar una comunicación completa e identificar posibles errores. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Emisor:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres dibujar una comunicación completa e identificar posibles errores y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con emisor, receptor, mensaje, canal y protocolo. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a representar teléfono → radio → HC-02 → UART → Mega. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: cada frontera y formato de mensaje quedan explícitos. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
El resultado normal es **cada frontera y formato de mensaje quedan explícitos**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **emisor** y dónde aparece en PX-32.
...
Hoy aprendiste a **dibujar una comunicación completa e identificar posibles errores** y lo conectaste con **emisor, receptor, mensaje, canal y protocolo**. Pudiste observar cada frontera y formato de mensaje quedan explícitos. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 52: Qué hace Bluetooth](52-que-hace-bluetooth.md).
