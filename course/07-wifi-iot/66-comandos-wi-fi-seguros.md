# Lección 66 — Comandos Wi-Fi seguros

## 1. Tu misión de hoy

Hoy vas a **aplicar F/B/L/R/S recibidos por red con las mismas reglas de STOP**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 57: Control Bluetooth con parada dominante](../06-bluetooth/57-control-bluetooth-con-parada-dominante.md), [Lección 65: Conectar en modo estación](65-conectar-en-modo-estacion.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **validación, comando, origen y caducidad**. En lenguaje cotidiano, buscamos una forma fiable de aplicar F/B/L/R/S recibidos por red con las mismas reglas de STOP. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Validación:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres aplicar F/B/L/R/S recibidos por red con las mismas reglas de STOP y qué mecanismo produciría ese resultado?
...
1. **Preparar.** Coloca PX-32 estable, identifica HW-003 a HW-006 y confirma con el adulto que la energía está en el estado seguro. Continúa solo si no hay cables sueltos, daño, calor u olor.
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con validación, comando, origen y caducidad. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a probar parser con motores deshabilitados y añadir timeout. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: solo comandos válidos y recientes pueden solicitar movimiento. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [66-comandos-wi-fi-seguros.ino](../../code/educational/66-comandos-wi-fi-seguros/66-comandos-wi-fi-seguros.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **validación**. Lee el programa de arriba abajo y predice su salida.

```cpp
// Curso PX-32 — programa mínimo de la lección 66
...
El resultado normal es **solo comandos válidos y recientes pueden solicitar movimiento**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **validación** y dónde aparece en PX-32.
...
Hoy aprendiste a **aplicar F/B/L/R/S recibidos por red con las mismas reglas de STOP** y lo conectaste con **validación, comando, origen y caducidad**. Pudiste observar solo comandos válidos y recientes pueden solicitar movimiento. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 67: Telemetría: el robot informa](67-telemetria-el-robot-informa.md).
