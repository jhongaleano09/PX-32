# Lección 60 — Una red está hecha de nodos y enlaces

## 1. Tu misión de hoy

Hoy vas a **dibujar la red local sin incluir datos privados**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 59: El ESP8266/ESP-12S](59-el-esp8266-esp-12s.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **red, nodo, enlace, paquete y topología**. En lenguaje cotidiano, buscamos una forma fiable de dibujar la red local sin incluir datos privados. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Red:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres dibujar la red local sin incluir datos privados y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con red, nodo, enlace, paquete y topología. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a representar PX-32, router y computador con flechas de paquetes. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: nodos y enlaces diferenciados; Internet no se supone necesario para control local. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
El resultado normal es **nodos y enlaces diferenciados; Internet no se supone necesario para control local**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **red** y dónde aparece en PX-32.
...
Hoy aprendiste a **dibujar la red local sin incluir datos privados** y lo conectaste con **red, nodo, enlace, paquete y topología**. Pudiste observar nodos y enlaces diferenciados; Internet no se supone necesario para control local. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 61: El router organiza la red local](61-el-router-organiza-la-red-local.md).
