# Lección 63 — Direcciones IP y DHCP

## 1. Tu misión de hoy

Hoy vas a **leer e interpretar una IP privada asignada al módulo**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 61: El router organiza la red local](61-el-router-organiza-la-red-local.md), [Lección 62: SSID y credenciales seguras](62-ssid-y-credenciales-seguras.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **IP privada, DHCP, puerto y dirección**. En lenguaje cotidiano, buscamos una forma fiable de leer e interpretar una IP privada asignada al módulo. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Ip privada:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres leer e interpretar una IP privada asignada al módulo y qué mecanismo produciría ese resultado?
...
1. **Preparar.** Coloca PX-32 estable, identifica HW-003 y confirma con el adulto que la energía está en el estado seguro. Continúa solo si no hay cables sueltos, daño, calor u olor.
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con IP privada, DHCP, puerto y dirección. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a consultar estado AT y distinguir dirección del dispositivo y puerto. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: dirección privada reconocida como identificador local temporal. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [63-direcciones-ip-y-dhcp.ino](../../code/educational/63-direcciones-ip-y-dhcp/63-direcciones-ip-y-dhcp.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **IP privada**. Lee el programa de arriba abajo y predice su salida.

```cpp
// Curso PX-32 — programa mínimo de la lección 63
...
El resultado normal es **dirección privada reconocida como identificador local temporal**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **IP privada** y dónde aparece en PX-32.
...
Hoy aprendiste a **leer e interpretar una IP privada asignada al módulo** y lo conectaste con **IP privada, DHCP, puerto y dirección**. Pudiste observar dirección privada reconocida como identificador local temporal. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 64: Cliente, servidor y UDP](64-cliente-servidor-y-udp.md).
