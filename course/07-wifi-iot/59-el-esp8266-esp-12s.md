# Lección 59 — El ESP8266/ESP-12S

## 1. Tu misión de hoy

Hoy vas a **explicar el papel del ESP8266 frente a la Mega2560**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 58: Wi-Fi y Bluetooth no son lo mismo](58-wi-fi-y-bluetooth-no-son-lo-mismo.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **SoC, firmware AT, antena y coprocesador**. En lenguaje cotidiano, buscamos una forma fiable de explicar el papel del ESP8266 frente a la Mega2560. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Soc:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres explicar el papel del ESP8266 frente a la Mega2560 y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con SoC, firmware AT, antena y coprocesador. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a localizar antena, módulo y ruta E con todo apagado. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: Mega como programa principal y ESP como interfaz de red en este montaje. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
El resultado normal es **Mega como programa principal y ESP como interfaz de red en este montaje**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **SoC** y dónde aparece en PX-32.
...
Hoy aprendiste a **explicar el papel del ESP8266 frente a la Mega2560** y lo conectaste con **SoC, firmware AT, antena y coprocesador**. Pudiste observar Mega como programa principal y ESP como interfaz de red en este montaje. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 60: Una red está hecha de nodos y enlaces](60-una-red-esta-hecha-de-nodos-y-enlaces.md).
