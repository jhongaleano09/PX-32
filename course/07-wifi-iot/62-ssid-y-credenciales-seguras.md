# Lección 62 — SSID y credenciales seguras

## 1. Tu misión de hoy

Hoy vas a **preparar un ejemplo de configuración sin versionar datos reales**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 61: El router organiza la red local](61-el-router-organiza-la-red-local.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **SSID, contraseña, secreto y archivo ignorado**. En lenguaje cotidiano, buscamos una forma fiable de preparar un ejemplo de configuración sin versionar datos reales. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Ssid:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres preparar un ejemplo de configuración sin versionar datos reales y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con SSID, contraseña, secreto y archivo ignorado. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a usar MI_RED y NO_ES_UNA_CLAVE_REAL y revisar .gitignore. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: ninguna credencial, ubicación ni IP pública queda en archivos rastreados. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
El resultado normal es **ninguna credencial, ubicación ni IP pública queda en archivos rastreados**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **SSID** y dónde aparece en PX-32.
...
Hoy aprendiste a **preparar un ejemplo de configuración sin versionar datos reales** y lo conectaste con **SSID, contraseña, secreto y archivo ignorado**. Pudiste observar ninguna credencial, ubicación ni IP pública queda en archivos rastreados. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 63: Direcciones IP y DHCP](63-direcciones-ip-y-dhcp.md).
