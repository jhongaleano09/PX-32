# Lección 64 — Cliente, servidor y UDP

## 1. Tu misión de hoy

Hoy vas a **explicar el recorrido de un comando UDP local**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 63: Direcciones IP y DHCP](63-direcciones-ip-y-dhcp.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **cliente, servidor, datagrama, UDP y puerto**. En lenguaje cotidiano, buscamos una forma fiable de explicar el recorrido de un comando UDP local. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Cliente:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres explicar el recorrido de un comando UDP local y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con cliente, servidor, datagrama, UDP y puerto. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a enviar un mensaje de prueba sin motores y observarlo por Serial. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: mensaje recibido completo o pérdida tolerada sin efecto peligroso. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [64-cliente-servidor-y-udp.ino](../../code/educational/64-cliente-servidor-y-udp/64-cliente-servidor-y-udp.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **cliente**. Lee el programa de arriba abajo y predice su salida.

```cpp
// Curso PX-32 — programa mínimo de la lección 64
...
El resultado normal es **mensaje recibido completo o pérdida tolerada sin efecto peligroso**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **cliente** y dónde aparece en PX-32.
...
Hoy aprendiste a **explicar el recorrido de un comando UDP local** y lo conectaste con **cliente, servidor, datagrama, UDP y puerto**. Pudiste observar mensaje recibido completo o pérdida tolerada sin efecto peligroso. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 65: Conectar en modo estación](65-conectar-en-modo-estacion.md).
