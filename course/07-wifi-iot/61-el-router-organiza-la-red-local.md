# Lección 61 — El router organiza la red local

## 1. Tu misión de hoy

Hoy vas a **describir cómo PX-32 se une a una red doméstica**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 60: Una red está hecha de nodos y enlaces](60-una-red-esta-hecha-de-nodos-y-enlaces.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **router, punto de acceso, cliente y puerta de enlace**. En lenguaje cotidiano, buscamos una forma fiable de describir cómo PX-32 se une a una red doméstica. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Router:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres describir cómo PX-32 se une a una red doméstica y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con router, punto de acceso, cliente y puerta de enlace. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a seguir un paquete local desde controlador hasta robot. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: ruta lógica correcta y límites entre Wi-Fi local e Internet. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
El resultado normal es **ruta lógica correcta y límites entre Wi-Fi local e Internet**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **router** y dónde aparece en PX-32.
...
Hoy aprendiste a **describir cómo PX-32 se une a una red doméstica** y lo conectaste con **router, punto de acceso, cliente y puerta de enlace**. Pudiste observar ruta lógica correcta y límites entre Wi-Fi local e Internet. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 62: SSID y credenciales seguras](62-ssid-y-credenciales-seguras.md).
