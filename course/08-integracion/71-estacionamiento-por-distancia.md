# Lección 71 — Estacionamiento por distancia

## 1. Tu misión de hoy

Hoy vas a **mantener una separación objetivo con pulsos cortos**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 49: Evitar obstáculos](../05-ultrasonido/49-evitar-obstaculos.md), [Lección 69: La seguridad tiene prioridad](69-la-seguridad-tiene-prioridad.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **consigna, error, tolerancia y control proporcional simple**. En lenguaje cotidiano, buscamos una forma fiable de mantener una separación objetivo con pulsos cortos. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Consigna:** idea principal que podrás reconocer en la actividad.
...
**Hardware:** HW-004 a HW-006, HW-009.
...
1. ¿Qué esperas observar cuando logres mantener una separación objetivo con pulsos cortos y qué mecanismo produciría ese resultado?
...
1. **Preparar.** Coloca PX-32 estable, identifica HW-004 a HW-006, HW-009 y confirma con el adulto que la energía está en el estado seguro. Continúa solo si no hay cables sueltos, daño, calor u olor.
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con consigna, error, tolerancia y control proporcional simple. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a simular error y después acercarse lentamente a un objeto ancho. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: avance lejos, STOP dentro de banda y retroceso solo bajo condiciones verificadas. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [71-estacionamiento-por-distancia.ino](../../code/educational/71-estacionamiento-por-distancia/71-estacionamiento-por-distancia.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **consigna**. Lee el programa de arriba abajo y predice su salida.
...
- Cuéntale con tus palabras qué significa **consigna** y dónde aparece en PX-32.
...
Hoy aprendiste a **mantener una separación objetivo con pulsos cortos** y lo conectaste con **consigna, error, tolerancia y control proporcional simple**. Pudiste observar avance lejos, STOP dentro de banda y retroceso solo bajo condiciones verificadas. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 72: Navegar un laberinto](72-navegar-un-laberinto.md).
