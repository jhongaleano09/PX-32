# Lección 72 — Navegar un laberinto

## 1. Tu misión de hoy

Hoy vas a **elegir una dirección usando medidas izquierda-centro-derecha**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 49: Evitar obstáculos](../05-ultrasonido/49-evitar-obstaculos.md), [Lección 68: Pensar en modos y estados](68-pensar-en-modos-y-estados.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **estrategia, heurística, memoria y callejón sin salida**. En lenguaje cotidiano, buscamos una forma fiable de elegir una dirección usando medidas izquierda-centro-derecha. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Estrategia:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres elegir una dirección usando medidas izquierda-centro-derecha y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con estrategia, heurística, memoria y callejón sin salida. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a resolver primero un laberinto de papel y luego un tramo físico simple. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: decisiones consistentes y recuperación mediante STOP/retroceso limitado. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [72-navegar-un-laberinto.ino](../../code/educational/72-navegar-un-laberinto/72-navegar-un-laberinto.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **estrategia**. Lee el programa de arriba abajo y predice su salida.

```cpp
// Curso PX-32 — programa mínimo de la lección 72
...
El resultado normal es **decisiones consistentes y recuperación mediante STOP/retroceso limitado**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **estrategia** y dónde aparece en PX-32.
...
Hoy aprendiste a **elegir una dirección usando medidas izquierda-centro-derecha** y lo conectaste con **estrategia, heurística, memoria y callejón sin salida**. Pudiste observar decisiones consistentes y recuperación mediante STOP/retroceso limitado. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 73: Línea con obstáculos](73-linea-con-obstaculos.md).
