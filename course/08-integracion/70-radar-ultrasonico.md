# Lección 70 — Radar ultrasónico

## 1. Tu misión de hoy

Hoy vas a **construir un mapa angular de distancias sin llamarlo radar físico exacto**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 48: Escaneo espacial](../05-ultrasonido/48-escaneo-espacial.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **radar como analogía, barrido, tabla y visualización**. En lenguaje cotidiano, buscamos una forma fiable de construir un mapa angular de distancias sin llamarlo radar físico exacto. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Radar como analogía:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres construir un mapa angular de distancias sin llamarlo radar físico exacto y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con radar como analogía, barrido, tabla y visualización. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a barrer 30°–150° por pasos y transmitir pares por Serial. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: perfil angular repetible y explicación de resolución/tiempo. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [70-radar-ultrasonico.ino](../../code/educational/70-radar-ultrasonico/70-radar-ultrasonico.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **radar como analogía**. Lee el programa de arriba abajo y predice su salida.
...
El resultado normal es **perfil angular repetible y explicación de resolución/tiempo**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **radar como analogía** y dónde aparece en PX-32.
...
Hoy aprendiste a **construir un mapa angular de distancias sin llamarlo radar físico exacto** y lo conectaste con **radar como analogía, barrido, tabla y visualización**. Pudiste observar perfil angular repetible y explicación de resolución/tiempo. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 71: Estacionamiento por distancia](71-estacionamiento-por-distancia.md).
