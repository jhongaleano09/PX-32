# Lección 69 — La seguridad tiene prioridad

## 1. Tu misión de hoy

Hoy vas a **hacer que un obstáculo anule una orden de avance**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
El tema de hoy es **prioridad, interbloqueo, fallo seguro y arbitraje**. En lenguaje cotidiano, buscamos una forma fiable de hacer que un obstáculo anule una orden de avance. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Prioridad:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres hacer que un obstáculo anule una orden de avance y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con prioridad, interbloqueo, fallo seguro y arbitraje. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a inyectar combinaciones de orden y distancia antes de mover. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: STOP domina MANUAL/AUTO y sensor inválido se trata como riesgo. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [69-la-seguridad-tiene-prioridad.ino](../../code/educational/69-la-seguridad-tiene-prioridad/69-la-seguridad-tiene-prioridad.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **prioridad**. Lee el programa de arriba abajo y predice su salida.

```cpp
// Curso PX-32 — programa mínimo de la lección 69
...
El resultado normal es **STOP domina MANUAL/AUTO y sensor inválido se trata como riesgo**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **prioridad** y dónde aparece en PX-32.
...
Hoy aprendiste a **hacer que un obstáculo anule una orden de avance** y lo conectaste con **prioridad, interbloqueo, fallo seguro y arbitraje**. Pudiste observar STOP domina MANUAL/AUTO y sensor inválido se trata como riesgo. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 70: Radar ultrasónico](70-radar-ultrasonico.md).
