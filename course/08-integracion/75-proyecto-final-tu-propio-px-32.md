# Lección 75 — Proyecto final: tu propio PX-32

## 1. Tu misión de hoy

Hoy vas a **diseñar, probar y presentar una función integrada segura**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 68: Pensar en modos y estados](68-pensar-en-modos-y-estados.md), [Lección 69: La seguridad tiene prioridad](69-la-seguridad-tiene-prioridad.md), [Lección 70: Radar ultrasónico](70-radar-ultrasonico.md), [Lección 71: Estacionamiento por distancia](71-estacionamiento-por-distancia.md), [Lección 72: Navegar un laberinto](72-navegar-un-laberinto.md), [Lección 73: Línea con obstáculos](73-linea-con-obstaculos.md), [Lección 74: Telemetría integrada](74-telemetria-integrada.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **requisito, prototipo, prueba, evidencia e iteración**. En lenguaje cotidiano, buscamos una forma fiable de diseñar, probar y presentar una función integrada segura. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Requisito:** idea principal que podrás reconocer en la actividad.
...
**Hardware:** Según el diseño aprobado.
...
1. ¿Qué esperas observar cuando logres diseñar, probar y presentar una función integrada segura y qué mecanismo produciría ese resultado?
...
1. **Preparar.** Coloca PX-32 estable, identifica Según el diseño aprobado y confirma con el adulto que la energía está en el estado seguro. Continúa solo si no hay cables sueltos, daño, calor u olor.
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con requisito, prototipo, prueba, evidencia e iteración. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a definir criterios, construir en pasos, probar fallos y hacer una demostración conversada. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: proyecto reproducible que entra en STOP ante pérdida de señal o dato inseguro. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [75-proyecto-final-tu-propio-px-32.ino](../../code/educational/75-proyecto-final-tu-propio-px-32/75-proyecto-final-tu-propio-px-32.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **requisito**. Lee el programa de arriba abajo y predice su salida.

```cpp
// Curso PX-32 — programa mínimo de la lección 75
...
El resultado normal es **proyecto reproducible que entra en STOP ante pérdida de señal o dato inseguro**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **requisito** y dónde aparece en PX-32.
...
Hoy aprendiste a **diseñar, probar y presentar una función integrada segura** y lo conectaste con **requisito, prototipo, prueba, evidencia e iteración**. Pudiste observar proyecto reproducible que entra en STOP ante pérdida de señal o dato inseguro. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. Has completado la ruta. El siguiente paso es repetir la demostración, explicar decisiones y elegir una mejora segura..
