...
Hoy vas a **configurar Arduino IDE 2 y verificar un programa sin mover el robot**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 04: La Mega2560: una computadora pequeña](../00-fundamentos/04-la-mega2560-una-computadora-pequena.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **IDE, compilar, placa, puerto y cargar**. En lenguaje cotidiano, buscamos una forma fiable de configurar Arduino IDE 2 y verificar un programa sin mover el robot. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Ide:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres configurar Arduino IDE 2 y verificar un programa sin mover el robot y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con IDE, compilar, placa, puerto y cargar. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a seleccionar Mega 2560, identificar el puerto y compilar Blink. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: mensaje de compilación correcta y puerto identificado con una prueba de desconexión. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
El resultado normal es **mensaje de compilación correcta y puerto identificado con una prueba de desconexión**. Puede haber variación por tolerancias, superficie, luz, fricción, carga, eco o tiempos del programa. Una variación pequeña y repetible es información; un salto grande, un reinicio, una lectura imposible o un movimiento inesperado exige STOP.
...
- Cuéntale con tus palabras qué significa **IDE** y dónde aparece en PX-32.
...
Hoy aprendiste a **configurar Arduino IDE 2 y verificar un programa sin mover el robot** y lo conectaste con **IDE, compilar, placa, puerto y cargar**. Pudiste observar mensaje de compilación correcta y puerto identificado con una prueba de desconexión. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 06: Primer programa: Blink](06-primer-programa-blink.md).
