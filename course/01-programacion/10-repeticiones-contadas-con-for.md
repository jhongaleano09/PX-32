...
Hoy vas a **producir grupos exactos de destellos sin copiar instrucciones**. Al terminar podrás demostrarlo con una explicación, un dato o un comportamiento observable; no basta con decir “funcionó”.
...
[Lección 09: Decisiones con if y else](09-decisiones-con-if-y-else.md). Debes poder explicar su idea central y repetir su prueba segura antes de continuar.
...
El tema de hoy es **bucle, contador, condición y actualización**. En lenguaje cotidiano, buscamos una forma fiable de producir grupos exactos de destellos sin copiar instrucciones. La palabra “fiable” importa: una sola coincidencia puede ser suerte; una explicación científica conecta una causa, una prueba y un resultado que otra persona podría repetir.
...
- **Bucle:** idea principal que podrás reconocer en la actividad.
...
1. ¿Qué esperas observar cuando logres producir grupos exactos de destellos sin copiar instrucciones y qué mecanismo produciría ese resultado?
...
2. **Trazar.** Señala la ruta entrada → proceso → salida relacionada con bucle, contador, condición y actualización. Si no puedes justificar un pin, consulta el mapa; no adivines.
3. **Predecir.** Elige un resultado concreto y una señal de parada. Di qué variable cambiarás y cuáles permanecerán iguales.
4. **Probar.** Vas a programar tres grupos de 2, 4 y 6 destellos. Haz un solo cambio. Observa antes de repetir y mantén accesible la forma de detener la prueba.
5. **Comprobar.** El resultado que permite continuar es: cantidad exacta de repeticiones y regreso controlado al inicio. Si no aparece, apaga cuando corresponda y pasa a “Si no funciona”.
...
Abre [10-repeticiones-contadas-con-for.ino](../../code/educational/10-repeticiones-contadas-con-for/10-repeticiones-contadas-con-for.ino). Antes de cargarlo, localiza `setup()`, `loop()` y la línea que representa **bucle**. Lee el programa de arriba abajo y predice su salida.
...
- Cuéntale con tus palabras qué significa **bucle** y dónde aparece en PX-32.
...
Hoy aprendiste a **producir grupos exactos de destellos sin copiar instrucciones** y lo conectaste con **bucle, contador, condición y actualización**. Pudiste observar cantidad exacta de repeticiones y regreso controlado al inicio. La regla de seguridad es cambiar conexiones únicamente sin energía y usar `STOP` ante información dudosa. La próxima sesión será la [Lección 11: Funciones: enseñar una acción reutilizable](11-funciones-ensenar-una-accion-reutilizable.md).
